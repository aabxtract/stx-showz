    ...
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="md:hidden p-3 -mr-1 rounded-lg hover:bg-slate-100 active:bg-slate-100"
          ...
          <div className="px-4 py-3 flex flex-col gap-1">
            {links.map((l) => (
              <Link
                ...
                className="px-3 py-3 rounded-lg text-sm text-slate-700 hover:bg-slate-100 active:bg-slate-100"
              >
                {l.label}
              </Link>
            ))}
